...
`index.html` is the small, one-file launcher for web-based HTML code runners. It fetches the complete NEO OS browser build from jsDelivr, so it does not depend on JavaScript being enabled for `file:///` pages. The launcher is pinned to commit `c36045eeec969b457372f7fc3db63ebf1c967aa4`, the current optimized NEO OS build, so CDN branch caching cannot mix versions.
...
- `neo-os/` — the complete optimized NEO OS build, including its wallpapers and browser runtime.
