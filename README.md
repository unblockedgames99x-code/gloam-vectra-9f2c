...
`index.html` is the small, one-file launcher for web-based HTML code runners. It fetches the verified NEO OS build from jsDelivr, so it does not depend on JavaScript being enabled for `file:///` pages. The launcher is pinned to commit `1705efd` so CDN branch caching cannot mix old catalog files with new ones.
